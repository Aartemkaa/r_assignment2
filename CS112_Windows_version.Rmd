...
From both graphs, it is clear that taking an expected value results in much more confident prediction. This happens because in addition to iterating over the probability distributon of the
...
"age": 
    LWB = 2.816e-3 - (1.96*3.314e-3) = -0.00367; 
    UPB = 2.816e-3 + (1.96*3.314e-3) =  0.00931.
    
"education":
    LWB = 1.629e-02 - 1.96*1.318e-02 = -0.00954;
    UPB = 1.629e-02 + 1.96*1.318e-02 =  0.04212

This is 95% confidence interval for both predictors. Since the Standard Error multiplied by 2 is bigger than the predicted coefficient, we can state that the predictors are weak and indicate low level of certainty about our predictions.
...
for (i in 3:16){
  ...
  model.glm.sim = sim(model.glm, n.sims=1000)
  ...
  for (x in 1:1000) {
      ...
      # vector containing the predictions for simulated coefficients
      preds = sum(model.glm.sim@coef[x,] * data_means)
      # preds = sigmoid(preds + model.glm.sim@coef[z, 1])
      preds = sigmoid(preds)
        
      # Expected Value is the mean of all the values obtained
      # using simulated coefficients
      expected_values = append(expected_values, preds)
  }
...
for (z in 1:100) {
        ...
        # getting the sum of the coefficients multiplied by the respective 
        # values ([2:7]) and adding the intercept to the resulting value
        preds = sum(model.glm.sim@coef[z, 2:6] * data_means) +
              model.glm.sim@coef[z, 1] + rnorm(1, mean=0, sd=model.glm.sim@sigma)
        # preds = sigmoid(preds + model.glm.sim@coef[z, 1])
        preds = logit2prob(preds)
        
        # storing all the predictions in the form of probabilities
        storage = append(storage, preds)
      }
...
start = mdy(c(dataset$launched))
end   = mdy_hms(dataset$deadline)

difference = 

dataset$length = c(as.double(difftime(lubridate::ymd_hms(dataset$deadline),
                   lubridate::ymd_hms(dataset$launched),
                   units = "days")))
