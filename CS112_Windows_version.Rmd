...
Write out hand calculations here. ### calculate it later!!!!!
...
```{r}
data(lalonde)
model.glm = glm(treat ~ age+educ+hisp+re74+re75, data=lalonde, family="binomial")
summary(model.glm)
```
...
for (i in 3:16){
  ...
  for (x in 1:5) {
      ...
      # Using 1000 values for sim() or as the number of simulation takes too 
      # much time to run, 100 values for both should suffice
      model.glm.sim = sim(model.glm.bootstrapped, n.sims=100)
