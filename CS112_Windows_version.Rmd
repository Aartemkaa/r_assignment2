...
for (i in 3:16){
  ...
  # getting a vector to store 1000 Expected Values
  # for a single value of the Education
  expected_values = c()
  ...
  for (x in 1:10) {
      ...
      for (z in 1:1000){
        predictions = model.lalonde.sim@coef[z, ]*data_means
        storage = append(storage, sum(predictions))
      }
  ...
  big    = append(big, quantile(expected_values, probs=0.975))
      ...
      #generate exp value from the storage vector
        # 1000 EV
