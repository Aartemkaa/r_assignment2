...
for (i in 3:16){
  ...
  for (x in 1:100) {
      ...
      #cat("Simulated", x, "sets of 1000 coefficients. \n")
  }
...
bootstrap = function(dataframe, samples) {
  
  # creating a new, bootstrapped dataframe
  bootstrapped = data.frame()
  
  for (column in 1:dim(dataframe)[2]) {
    # getting bootstrapped values
    values = sample(dataframe[, column], size=samples, replace=TRUE)
    
    # assigning the values to a column with identical name in a new dataframe
    bootstrapped$names(dataframe[column]) = values
  }
  
  return(bootstrapped)
}

model.glm.bootstrapped = glm(treat ~ age+educ+hisp+re74+re75, data=bootstrapped, family="binomial")
